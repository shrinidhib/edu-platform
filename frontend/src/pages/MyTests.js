...
const Tests = () => {
    ...
    const fetchTests=async()=>{
        const response=await fetch(`https://edu-frontend-sage.vercel.app/test/mytests/${user.user._id}`,{
            method: 'GET',
            headers:{
    ...
    const deleteHandler=async(id)=>{
        ...
        if (isConfirmed){
            const response=await fetch(`https://edu-frontend-sage.vercel.app/test/${id}`,{
            method: 'DELETE',
            headers:{
                'content-type':'application/json',
                "Authorization":`Bearer ${user.token}`
