...
const DisplayScores = ({ userId }) => {
    ...
    const fetchTestTitles = async () => {
      ...
      await Promise.all(
        latestScores.map(async (score) => {
          try {
            const response = await axios.get(`http://localhost:4005/test/title/${score.testID}`);
            titles[score.testID] = response.data.title;
            console.log(`Fetched title for testID ${score.testID}:`, response.data.title);
          } catch (error) {
  ...
  return (
    <div className="display-scores-container">
      <h2>Scores for User ID: Vardhan</h2>
